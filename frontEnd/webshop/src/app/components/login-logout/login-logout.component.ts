...
export class LoginLogoutComponent implements OnDestroy {
  ...
  userIntent() {
    ...
    if (this.auth.hasToken()) {
      ...
      this.cartService.emptySavedCart();
      ...
      if (
        ...
        this.userObject.password === undefined
      ) {
      ...
      this.registerSubscription = this.auth.register(this.userObject).subscribe(
        endPointResponseObj => {
        ...
        error => {
          console.log(
            "error @ login-logout.component.ts register service observer: "
          );
          console.log(error);
      ...
      if (
        this.userObject.email === undefined ||
        this.userObject.password === undefined ||
        typeof this.userObject.email !== "string"
      ) {
        console.log(" email and password is needed ");
        return window.alert(" email and password is needed ");
      }
